...
<main>
  ...
  <div class="filtros">
    ...
    <select class="sel" id="selMarca"></select>
    <select class="sel" id="selMedio"></select>
  ...
  <div class="filtros" style="margin-top:-8px;padding-top:12px;padding-bottom:12px">
    <span style="font-size:11.5px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.5px">Medios</span>
...
// ---- estado de filtros ----
const state = { q:"", medios:new Set(ORDEN), marca:"all", soloOp:false, seccion:"all", sortMarca:["marca","asc"], sortNota:["fecha","desc"] };

// marcas disponibles
const marcasLista = [...new Set(DATA.notas_comerciales.map(n=>n.marca).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"es"));
document.getElementById("selMarca").innerHTML =
  `<option value="all">Todas las marcas</option>` + marcasLista.map(m=>`<option value="${m}">${m}</option>`).join("");

// medios disponibles (dropdown alternativo a los chips)
document.getElementById("selMedio").innerHTML =
  `<option value="all">Todos los medios</option>` + ORDEN.map(k=>`<option value="${k}">${MEDIOS[k]}</option>`).join("");
...
function notasFiltradas(){
  ...
  return DATA.notas_comerciales.filter(n=>{
    ...
    if(state.marca!=="all" && n.marca!==state.marca) return false;
...
document.getElementById("selMarca").addEventListener("change", e=>{state.marca=e.target.value; render();});
document.getElementById("selSeccion").addEventListener("change", e=>{state.seccion=e.target.value; render();});

function syncChips(){ document.querySelectorAll("[data-medio]").forEach(c=>
  c.classList.toggle("on", state.medios.has(c.dataset.medio))); }

document.getElementById("selMedio").addEventListener("change", e=>{
  state.medios = e.target.value==="all" ? new Set(ORDEN) : new Set([e.target.value]);
  syncChips(); render();
});
document.querySelectorAll("[data-medio]").forEach(ch=>ch.addEventListener("click",()=>{
  ...
  document.getElementById("selMedio").value="all";   // los chips mandan: el dropdown vuelve a "todos"
  render();
}));
...
document.getElementById("btnClr").addEventListener("click",()=>{
  state.q=""; state.medios=new Set(ORDEN); state.marca="all"; state.soloOp=false; state.seccion="all";
  document.getElementById("q").value="";
  document.getElementById("selMarca").value="all";
  document.getElementById("selMedio").value="all";
  document.getElementById("selSeccion").value="all";
